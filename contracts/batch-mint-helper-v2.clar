...
(define-public (batch-mint-jackpot (entries (list 100 { recipient: principal, amount: uint })))
  (ok (map mint-jackpot-single entries))
)

(define-private (mint-jackpot-single (entry { recipient: principal, amount: uint }))
  (contract-call? .jackpot-token mint (get amount entry) (get recipient entry))
)
